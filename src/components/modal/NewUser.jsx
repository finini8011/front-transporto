...
import { useNewUserMutation } from "../../api/services/auth/apiSlice";
...
const NewUser = ({ closeModal }) => {
  ...
  const [saveUser, { isLoading, error }] = useNewUserMutation();
  ...
  const onSubmit = async (dataForm) => {
    ...
    const { email, password, name, password_confirmation } = dataForm;
    ...
    if (password === "" || name === "" || email === "") return toast.error("Llenar todos los campos");
  ...
  return (
    <form>
      ...
      <ButtonPrimary loading={isLoading} text="Guardar" onClick={handleSubmit(onSubmit)} />
    </form>
  );
};
